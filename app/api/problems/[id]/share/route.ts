...
import { withDb, type DatabaseConnection } from "@/lib/db/init";
...
function ownedProblem(db: DatabaseConnection, id: string, userId: string): ShareRow | undefined {
...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    // withDb: heal a stale Turso stream and retry the whole share flow once.
    return withDb((db) => {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          ...
          throw err; // surfaced by the outer catch (redacted)
        }
      }
